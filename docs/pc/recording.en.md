...
!!! tip "By default, what you see in Rerun is what lands on disk"
    On disk = `--robot.tactile_output_types`, default `rectify` (the raw image with no baseline subtraction); exactly one type, more than one is an error. Display = `--robot.tactile_display_output_types`, also `rectify` by default (an empty list `'[]'` means the same), so each sensor is read once per frame and the screen and the dataset carry the same image. Point it at another type and that becomes a second output on the same read, display-only and never recorded; the key is shaped `tactile_left_difference` and is not in `observation_features`.

    The type worth knowing is `difference`, an enhanced difference image against the baseline captured at sensor init. It used to be the default: on the earlier gel, `rectify` showed too little deformation to read contact from live. The silicone was changed in 2026-08 and contact now reads directly off `rectify`, so the viewer no longer has to put a non-recorded stream in front of the operator. If you do turn it back on (`--robot.tactile_display_output_types='["difference"]'`), remember the difference image is destructive: the baseline is grabbed at sensor init, so any force pressing on the gel at connect time is subtracted out of the whole session. Keep all four fingertips unloaded at connect, and never switch `--robot.tactile_output_types` to `difference` just because it looks clearer. `--robot.tactile_diff_gain` (default `1.0`) is the gain on that difference image and does nothing while `difference` is not requested; the factory value of 1.5 is noisy on this gel and clips.
