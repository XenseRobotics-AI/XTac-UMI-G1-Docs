...
控制台程序的升级都在 控制台 → 系统 → 系统更新 页完成,版本以这一页显示为准(本文按 0.4.1 写)。页面从上到下是指标卡(当前版本、git、构建时间、架构、状态:空闲 / 已就绪 / 应用中 / 错误)与「上传固件包」「待应用版本」「回滚」「远程更新」四张卡。

![系统更新页](../assets/backpack/update.webp)

截图为旧版界面,当前页面在「回滚」右侧还有「云 OTA 下发」与「matrix 下发」两张远程更新卡。

## 导入升级包 {#bundle}

升级包是一个 `.tar.zst` 文件,由技术支持提供,文件名形如 `taccap-collector-v0.4.1-245dd270d88f-aarch64.tar.zst`:版本号在前,后面 12 位是 git hash,最后是架构。包里只有控制台程序(前后端打在同一个二进制里)和一份 `manifest.json`,不含夹爪固件、相机参数包与任何采集数据。因此,部分需要更新设备系统组件的修复,不会随这个控制台升级包安装,见[控制台升级包覆盖不到的更新](#system-patch)。
...
### 控制台升级包覆盖不到的更新 {#system-patch}

控制台升级包更新的是控制台程序本身。少数修复要改的是设备上的系统组件,「系统更新」页没有这个入口,只能由技术支持在设备上处理。

0.4.x 的网络改动就有这一部分:升级到新版之后,网络相关的新行为要等设备的系统组件也更新过才完整。设备上缺少这些组件时,升级本身照常完成,**网络设置保持原样**,不会把你正在用的连接改掉。

!!! warning "不是所有更新都能从「系统更新」页完成"
    控制台升级包不覆盖设备的系统组件。0.3.19 修的「插着 Pico 时背包的外网出口被 Pico 抢占」就属于这一类:把控制台程序升到 0.3.19 或更高之后,这项修复不一定同时生效,较早出厂的设备可能无法获得相应的系统修复。

    升到 0.4.x 时还要留意:网络设置的迁移**排在升级通过自检之后**才执行,所以刚重启完看到的可能还是旧设置,过一会儿才变。升级后连不上就按[网络与控制台入口](network.md)重新找一遍地址。

    这台设备要不要额外处理,**以技术支持的通知为准**。技术支持明确告知需要时由他们处理,现场不需要、也无法自行操作;控制台上看到的版本号仍然只代表控制台程序的版本。

    技术支持提供的其他文件**不要**传进「上传固件包」——那里只接受 `.tar.zst` 控制台升级包,传别的会被校验拒收。
...
本节只适用于已接入管理端的部署;未接入管理端的设备不会出现这些远程更新卡片,升级按上面的[导入升级包](#bundle)走。
