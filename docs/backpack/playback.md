...
能不能回放只看**原始文件还在不在设备上**,与传没传过无关:[已就绪与已上传](monitor-record.md#episode-state)的条目都能正常回放(0.3.11 修复,此前一条只要上传过一次,哪怕文件完好也会提示「未就绪」)。[已归档](monitor-record.md#episode-state)的条目原始文件已在归档时清理,项目页里它的「回放」按钮置灰、悬停写明「已归档,本地源已删除」,点不开也导不出;要看这批数据,去上传到的远端仓库取。
...
回放页上没有下载按钮:单条下载的两个入口(回放页的「下载 H264」、导出窗口里按条勾选下载)在 0.3.10 已一并移除,取回文件统一走任务级导出 —— 在[导出对话框](projects-export.md#export)里选去向「下载到设备」与格式,设备打好压缩包后一次取回。
