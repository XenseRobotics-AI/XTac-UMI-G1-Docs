...
后端种类有五种:**ModelScope**、**S3**(桶需预先建好)、**FTP / FTPS**、**NFS 网络存储**、**[STS(导出 MCAP)](#sts)**。前四档都收 `LeRobot 数据集` 与 `mcap` 两种格式,目录按同一套规则自动命名,不用手填:**`<根>/ 项目名称 / 模式前缀-任务名-日期 /`**(「根」按种类分别是 Owner、桶或目标根目录)。已经上传过的任务继续用它原来的目录。

要填哪些字段、怎么自查权限,都在 控制台 → 系统 → [上传配置](system.md#upload) 那一页,本页不重复。设备提供哪几档,以 系统 → 上传配置 页面里的后端选项为准。

上传**按录制日期分批**:同一天录的追加进同一个目录,跨天分开上传。导出对话框会先把这次要写进哪个目录预览出来,传之前可以核对。

保持当前后端会续写原仓库;改选其它上传后端时,只把尚未上传的新数据写入新仓库,并从第 0 条开始编号,以后切回原档仍能接着原仓库的进度往下写。每个远端仓库各有自己的续写游标,同一条录制在同一格式下只上传一次,换目标不会把旧批次重灌一遍。
...
导出或上传报错的处理见[故障排查](troubleshooting.md)。本页按 XTac-UMI Collector 0.4.1 编写,设备实际版本以 控制台 → 系统 → [设备信息](system.md#device-info) 页显示为准,各版本变化见[版本](versions.md#baseline)。
