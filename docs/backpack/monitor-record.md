...
录制条目落在「项目 → 任务」两级之下,监控页底部的两个下拉分别选项目和任务,末项分别是「新建项目...」「新建任务...」,可以就地新建;也可以在[项目页](projects-export.md)新建并把某个任务「设为当前目标」。**采集模式是项目的属性,建项目时定下就冻结**,不能中途切换,系统页只做只读展示,见[当前项目采集配置](system.md#capture-mode)。

新建项目除了名称,还要把这个项目的**采集配置**一次定下来:采集模式(六选一)、PICO 分辨率(带头显的模式才要选)、触觉导出方向、PICO 图像源(鱼眼原图 / 去畸变)与腕部鱼眼矫正。**建完就冻结,改不了** —— 想换一套参数要新建项目,各项的含义见[当前项目采集配置](system.md#capture-mode)。如果已在[系统 → 上传配置](system.md#upload)配好上传后端,还可以在这里给项目绑定一个,之后该项目发布时默认用它;不绑定就在发布时再选。
...
3. 夹爪 MCU 在线;
4. 带头显的模式:Pico 位姿、画面与时钟同步就绪;
5. 追踪器处于「视线内正常追踪」:静止 5 秒以上或移出视野时拒绝开录(录制中途短暂静止不会中断,也不计入质检,静止时位置仍然准确);
6. 头显的画面参数与项目对得上:选中项目或头显重连后,设备会自动重新读一遍头显的单目 / 双目、分辨率与图像源,开录前再核对一次。**没收到确认、核对超时,或这期间头显断过线、参数变过,都会拒绝开录**,提示里会写明是哪一种。
7. 头显的画面时钟正常:某只眼的视频时间比接收时间超前超过 1 秒,会提示「头显…眼视频时间同步异常」并拒绝开录 —— 这是头显应用的相机时钟与同步时钟对不上,不是背包的问题。

被拒时控制台弹「无法开始录制」窗口写明原因,按键开录被拒也弹同一个窗口;人不在平板旁就听语音、看灯语。录满时的原话是「任务「…」已达到累计采集目标(N/M),已拒绝开始录制。如需继续采集,请提高任务的目标条数,或删除不应计入目标的录制」。

### 录制中 {#record-live}

状态行变成「录制中」加走表时长,采集模式徽章仍在;顶栏「录制时长」同步走表。录制卡上用红字实时累计三项:「坏帧 N」(送达但内容残缺)、「掉线 N」(设备级断开、帧根本没送达)、「tracker 出视野 X s」(次数多于一次时带 ×N)。相机全绿时出视野是唯一的线索,看到就该考虑停下重录。录制中不能删除条目、不能改任务;采集模式属于项目、任何时候都不能切。
