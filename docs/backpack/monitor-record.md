...
本页讲 XTac-UMI Collector 控制台(下称控制台)的「实时监控」页:开录前要在这一屏上确认什么、项目与任务怎么选、录制怎么开始与停止、录坏的一条怎么当场删掉。夹爪按键的手势与灯语只在[夹爪按键与指示灯](../common/gripper.md#buttons)一处写,本页只链接。
...
| 磁盘余量 | 顶栏「硬盘」占用低于 80 % | 达到 80 % 会直接拒绝开录,先[发布并清盘](projects-export.md#modelscope)或删掉不要的录制 |
...
目标留空时监控页只显示已采数量、不画进度。填了目标条数就是硬门槛:录满即拒绝开录,控制台按钮和夹爪按键都会被挡。要继续采,提高任务的目标条数,或先发布已完成的录制:计数只算状态为「已就绪」的条目,发布并清盘后可以接着录下一批;「删除上条并重录」净条数不变,不受影响。

左下角「当前任务条数」卡显示当前任务已采条数、时长与对目标的进度。已删除的不计入;用夹爪按键或另一台平板开 / 停录,这张卡与项目页的列表也会即时更新。

腕部相机固定 640×480 @ 30 fps、触觉固定 640×480 @ 120 fps,都不可调;LeRobot 导出固定 30 fps,分辨率照抄采集不缩放。

## 录制 {#record}

日常以夹爪按键为主:右爪长按开始,左爪长按停止,由设备端状态机接管,不需要浏览器在线。手势表、灯语表与删除确认的按法见[夹爪按键与指示灯](../common/gripper.md#buttons),绑定爪位与时序是全机队固定值,现场不可调。控制台底部的「开始录制 / 结束录制」按钮做同一件事,给没拿夹爪的人用;想给这两个按钮绑浏览器快捷键,去[系统 → 按键绑定](system.md#keybinding)。
...
被标可疑的条目,当场用下一节的办法删掉重录,比事后在项目页翻要省事。
