...
- USB 网络(推荐):头显 Type-C 口已通过上面的接线连到背包 PICO 口。在 XR 里勾选「USB 网络」点「连接」,背包会自动在 USB 链路上建网(地址 `192.168.58.1`),不用手填。
- WiFi:头显与背包接同一台路由器(头显连路由器 WiFi,背包走网口或现场 WiFi,见[接入现场网络](network.md#lan)),XR 里不勾「USB 网络」,填背包的 IP 再连接。要事先知道背包 IP,且背包服务已开启。
