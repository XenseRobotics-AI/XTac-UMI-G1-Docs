...
??? failure "Spots on the image / blurry image"
    ...
    **Fix:** clean with a lint-free cloth, see [Maintenance](maintenance.md). A scratched or dented sensor must be replaced.
...
??? failure "No pose / the tracker will not connect / the pose is unstable"
    ...
    **Fix:** turn the data-collection host's WiFi off first and leave only the wired network sharing (see [Network connection](host-setup.md#pico-network)); then walk through the [Power-on sequence](quickstart.md#power-on) item by item; start the service with `/opt/apps/roboticsservice/runService.sh`; if needed, self-check with `python -m lerobot.robots.taccap_gripper.check_tracker`.
...
??? failure "The pose reference frame drifts between episodes"
    ...
    **Fix:** do not restart XTac-UMI XR between episodes. If it was restarted, treat the data recorded afterwards as a new dataset. See [Frame alignment](host-setup.md#pico-frame).
...
??? failure "The pose cuts out between episodes / the headset blanks its screen by itself"
    ...
    **Fix:** Enterprise settings → System settings → Power policy. Set "System sleep" to "Never" first, then set "Screen off" to "Never" (in the other order, blanking gets clamped back to a finite value by the sleep timeout). See [System settings](host-setup.md#pico-system).
...
??? failure "The tracker is not selectable in tracking mode / PC Service does not discover that SN / the pairing screen cannot find the tracker"
    ...
    **Fix:** open the "Motion Tracker" app and complete the binding, for both trackers. Before pairing, hold the power button for about 6 seconds until the indicator alternates blue and red, then tap "Start pairing". See [Binding the trackers](host-setup.md#pico-tracker-bind).
...
??? failure "XTac-UMI XR keeps showing \"not connected\""
    ...
    **Fix:** tap "Reconnect" first. If that does not help, redo the [Network connection](host-setup.md#pico-network) and confirm the computer's WiFi is off. The screen is shown in [The app's interface](host-setup.md#pico-toolkit-ui).
...
??? failure "The tracker is matched to the wrong side / PC Service enumeration is unstable"
    ...
    **Fix:** pin it literally with `--robot.tracker_serial=<SN>` (no enumeration, no validation); or check the digit before the trailing letter `G` in the serial number, odd is left, even is right, see [Tracker serial numbers](host-setup.md#pico-tracker-sn).
...
??? failure "Repeated left/right eye skew warnings in the log"
    ...
    **Fix:** the warning does not interrupt recording, but for those frames the two eyes may not be in sync. Reduce host load first (fewer cameras, or `--robot.head_camera_eyes=left` to record one eye only); for link problems work through [Network connection](host-setup.md#pico-network); only once you are sure it is just jitter, relax the threshold moderately.
...
Still stuck? Report it through the channels in [Support and feedback](reference.md#support), with the complete error, the self-check output (`scan_grippers`'s side / role / firmware_sn), version information and the command that reproduces it.
