...
!!! info "XTac-UMI G1 已验证环境"
    ...
    这台机器的**硬件配置**要求见下面一节 [采集主机配置要求](#host-spec)。

## 采集主机配置要求 {#host-spec}

采集是**实时**的:双夹爪六路相机边取流边编码,一帧只有 **33.3 ms** 的预算(30 fps)。
所以主机不是"能开机就行"——**配低了的表现是丢帧和录制效率低,而不是报错**,
事后从数据里也不容易看出来。
...
| USB | **单夹爪**(3 路相机)接在同一条 USB 2.0 总线上即可 | **双夹爪**(6 路相机)分挂**两条 USB 2.0 总线**,即两个独立 USB 主控制器 |
| 系统 | Ubuntu 22.04 / 24.04 LTS,**amd64** | 同左 |

!!! tip "NVIDIA 显卡决定的是录制效率"
    `--dataset.vcodec=auto` 会把 H.264 编码交给显卡上**专用的编码芯片**,CPU 只负责喂帧。
    没有 NVIDIA 显卡时这条路走不通,只能关掉流式编码改用 CPU 编码——**能录,但效率明显下降**
    (存盘变慢、更容易 `[slow_frame]`),见
    [没有 NVIDIA GPU 的主机怎么录](05-data-collection.md#no-gpu)。

    Docker 交付镜像则要求 NVIDIA 显卡与驱动。驱动版本用这条查:
...
!!! note "各项配置为什么是这个数"
    - **CPU** —— 采集主循环、触觉图像解码、给编码器喂帧都压在 CPU 上,双夹爪一帧要处理
      六到八张图。12 代 i7 是实测能稳住 30 fps 的下限。
    - **内存** —— 8 GB 只够跑采集本身。**开 `--display_data` 用 Rerun 看实时数据**、
      或者边采边处理数据时会很紧张,推荐 32 GB。
    - **硬盘** —— 满负荷时原始视频出流可达约 **280 MB/s**(双夹爪),
      落盘量另算,见[磁盘规划与估算](data-management.md#storage-planning)。
      **机械硬盘和 USB 移动硬盘不要用来直接录**。
    - **USB** —— 触觉与腕相机都是 USB 2.0 设备,每路相机打开时都要**独占预留**一份等时带宽,
      而一条 480M 总线的预算是固定的。**每只夹爪 3 路**(触觉 ×2 + 腕相机),
      双夹爪 6 路挤在同一条总线上会有相机**打不开**——这与 CPU / 显卡多快无关,
      插蓝色 USB 3 口也不解决。接线前先 `lsusb -t` 数一数每条 `480M` 总线上挂了几个,见
      [USB 带宽预算](03-host-hardware.md#usb-budget)。
...
| NVIDIA GPU | 装得上,但[最低配置](#host-spec)按 RTX 3060 / 8 GB 显存起——没有 NVIDIA 显卡时只能[降级录制](05-data-collection.md#no-gpu) | **必需**,驱动 ≥ 570.144 |
| 环境隔离 | 装在主机的 Mamba 环境里 | 装在容器里,不污染主机 |
| 改代码 | 方便 | 不方便 |
| 步骤 | 下面 2.1 – 2.5 | [Docker 交付镜像](#docker) |

**默认走 Mamba**——它对显卡驱动没有额外要求(**没装 NVIDIA 驱动的机器也装得上**),
