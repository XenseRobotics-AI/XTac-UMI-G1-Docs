...
硬件已连好、环境已装好、主夹爪已标定?这一页从上电到出第一条数据,照抄即可。第一次用请先走
[认识设备](device.md) → [安装](install.md) → [主机与 Pico4 配置](host-setup.md) →
[标定与自检](calibration.md),再回来。
...
3. 接好 Pico4 Ultra 企业版的有线共享网络,并关闭数采电脑的 WiFi——有线共享网络会与电脑 WiFi
   ...
   [网络连接](host-setup.md#pico-network)。
4. 开启 Pico4 Ultra 企业版,短按追踪器电源键至蓝灯亮起(首次使用需先[绑定](host-setup.md#pico-tracker-bind))。
...
6. 面朝机器人正前方,启动 XTac-UMI XR APP(冻结世界系原点与方向,见[坐标系](host-setup.md#pico-frame)),
   点「重连」使[状态变为「已连接」](host-setup.md#pico-toolkit-ui)。
7. 运行标定 / 自检 / 录制脚本。
...
下电按相反顺序:先停止录制 / 回放并等当前集存盘,再退出 XTac-UMI XR、停掉 XenseVR PC Service,最后按[认识设备](device.md)里的拔线顺序拔线:先拔数采终端端,再松螺钉拔夹爪端,从夹爪拔夹爪端前先断 24V。
