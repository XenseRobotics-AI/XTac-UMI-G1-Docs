...
追踪器不需要标定:安装变换内置,左右侧别由 SN 自动匹配。追踪器与夹爪的绑定在[主机与 Pico4 配置](host-setup.md#pico-tracker-bind)里做完;本页这条命令只读不写,只打印位姿,用来确认链路通了、装配没错。
...
以 10 Hz 打印 `raw`(追踪器自身位姿)与 `ee`(经刚性安装变换后的 TCP)。挥动夹爪,`raw xyz` 应平滑变化、SN 与预期一致([读取追踪器 SN](host-setup.md#pico-tracker-sn))。
