...
数采背包是一台随机附带的 RK3588 计算节点(eMMC 系统盘 + NVMe 数据盘),负责集中接收、缓存与落盘 XTac-UMI G1 和 Pico 系统的采集数据,并按项目、任务组织。它内置了 PC 版里跑在工作站上的 XTac-UMI XR 运行时,头显直接接到它身上。
