...
本页把 `xense-taccap-lerobot` 与三个硬件 SDK 装到 **Ubuntu 22.04 / 24.04 LTS(amd64)** 采集主机上并验证,装完就能跑 `lerobot-info`;串口权限与设备发现见下一页[主机与 Pico4 配置](host-setup.md)。
...
环境装好后,先做[主机与 Pico4 配置](host-setup.md)(串口权限、设备发现),否则夹爪能被列出却打不开。
