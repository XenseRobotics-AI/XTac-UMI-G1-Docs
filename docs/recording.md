...
`tcp.*` 是夹爪末端而非追踪器:追踪器离两指中点约 195 mm,落盘前乘上内置的刚性安装变换(CAD 装配实测,左右各一套),机体固连、任意姿态成立、与 `gripper.pos` 无关。世界系是重力对齐的 X 前 / Y 左 / Z 上,在 XTac-UMI XR 启动瞬间冻结(见[坐标系对齐](host-setup.md#pico-frame))。

### Rerun 里的 `/world` 3D 视图 {#world-view}

`--display_data=true` 会开出 `/world` 3D 视图:夹爪以带标签的椭球 + 三轴画在实时 `tcp.*` 处,拖出一条轨迹面包屑。
...
| ② 加追踪器位姿(最常用,本页默认) | `--robot.enable_tracker=true --robot.enable_head_camera=false` | 追踪器已开机[绑定](host-setup.md#pico-tracker-bind)、Pico4 已连上、[PC Service 已启动](host-setup.md#35) | [`/world` 视图](#world-view) / `tcp.*` |
...
| `fps` | `30` | 录制采样帧率上限;传感器本身 120 Hz([规格](device.md#specs)) |
...
采集全程不要重启 XTac-UMI XR,原因见[坐标系对齐](host-setup.md#pico-frame);不得不重启时,把之后的数据当作新数据集处理。
...
- 走有线时关闭数采电脑 WiFi,有线共享网络会与 WiFi 冲突,见[网络连接](host-setup.md#pico-network)。
