...
`tcp.*` is the gripper tip, not the tracker: the tracker sits about 195 mm from the two-finger midpoint, and before anything is written to disk it is multiplied by a built-in rigid mount transform (measured off the CAD assembly, one per side). The transform is body-fixed, holds in any orientation and is independent of `gripper.pos`. The world frame is gravity-aligned, X forward / Y left / Z up, and is frozen the instant XTac-UMI XR starts (see [frame alignment](host-setup.md#pico-frame)).

### The `/world` 3D view in Rerun {#world-view}

`--display_data=true` opens a `/world` 3D view: each gripper is drawn as a labelled ellipsoid plus an axis triad at its live `tcp.*`, trailing a breadcrumb of where it has been.

- The scene is declared `FLU`, the initial view looks down +X, and the world axes are labelled `+X forward / +Y left / +Z up`.
- The breadcrumb keeps the last 90 samples (about 3 s at 30 fps) and fades with age.
- With the head camera on, the headset is drawn into the same `/world` as a smaller amber `HEAD` marker, without a trail.
- `--show_trajectory` is on by default; `false` turns it off. It is skipped automatically when `--robot.enable_tracker=false`.
- Laid flat, the EE marker belongs at the two-finger midpoint with the triad X forward / Y left / Z up; otherwise the tracker is mounted wrong.
- `tracker pose` in the scalar panel is the tracker's raw pose, view only; what is recorded is `tcp.*`.
...
| ② Plus the tracker pose (the usual one, this page's default) | `--robot.enable_tracker=true --robot.enable_head_camera=false` | Tracker powered on and [bound](host-setup.md#pico-tracker-bind), Pico4 connected, [PC Service running](host-setup.md#35) | [`/world` view](#world-view) / `tcp.*` |
...
| `fps` | `30` | Recording sample-rate cap; the sensors themselves run at 120 Hz ([specs](device.md#specs)) |
...
Never restart XTac-UMI XR during collection; see [frame alignment](host-setup.md#pico-frame) for why. If you have no choice but to restart, treat everything after it as a new dataset.
...
- When using the wired link, turn the data-collection host's WiFi off; wired network sharing conflicts with WiFi. See [network](host-setup.md#pico-network).
