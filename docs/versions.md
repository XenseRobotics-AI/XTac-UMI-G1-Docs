...
| 固件构建号 | leader `≥ 1.2.0`<br>follower `≥ 1.1.0` | 具体那一版镜像。**这是最小值,不是必须精确匹配的值**——1.2.2 之类的更高版本同样支持 V2.1,不需要为此回刷。但更高的构建号可能修掉了[已知缺陷](#ota-when),那是另一回事 |
...
更高的构建号(如 leader `1.2.2`)同样支持。所以看到自己是 1.2.1 或 1.2.2 都不必疑惑,也不需要回刷到
...
!!! warning "固件镜像 1.2.2 / 1.1.5:这一次**建议刷**,和之前几次不一样"
    ...
    **前几次升级只改了 LED 颜色一类的表面行为,这一次不是。**1.2.2 / 1.1.5 修掉了三个实打实
    ...
    见[但 V2.1 只是能用的底线](#ota-when)。要求门槛不变(leader ≥ 1.2.0 / follower ≥ 1.1.0),
    变的是**推荐值**。

    两件事随之而来,刷之前先读:

    - 这两版镜像是**本地工具链编译**的,不是固件团队发布工具链的产物(`manifest.json` 的
      `build` 字段写着 `local`)。换来的是那三条修复,代价是少了那条发布链路的背书;
      两版都已在两台实机上验证过。
    - **刷完必须断电重插**,这是流程的一步,不是排障手段——见 [OTA](#ota) 里那条 danger。
...
!!! note "相机连不上要先装 `libusb-dev`,需要 `9a4d696b` 之后的版本"
    近期的 Linux 内核更新要求配套的 libusb;缺了或者版本落后,**相机就是连不上**,而报错里
    没有一个字提到 libusb——所以排查往往先去翻 USB hub、线材和带宽,方向全错。从这一版起
    [2. 环境部署](02-environment.md#apt) 把 `libusb-dev` 列成独立的安装项,`setup_env.sh`
    也会在缺 `libusb-0.1.so.4` 时告警(只告警不中断:编译期用不到它,装得上、连接时才失败)。

    `apt upgrade` 带来新内核之后,记得同步升一次:

    ```bash
    sudo apt update && sudo apt install -y --only-upgrade libusb-dev libusb-1.0-0
    ```

    同一版里 `usbutils` 从推荐列表中移除(相应地,`lsusb -t` 那半段排查说明也一并去掉了)。

!!! note "录到一半换硬件会记成一个新 epoch,需要 `052eb354` 之后的版本"
    `meta/hardware.json` 从"一份扁平的 `units`"变成 **`epochs` 数组**:每段记
    `from_episode` / `to_episode`(左闭右开)、`recorded_at`,以及那一段用的 `robot_id`、
    `role` 和 `units`。续录时如果换了夹爪或传感器,旧段在当前集数处**封口**,新段接上,
    每一集都还指向真正采它的那套设备。

    停在更早的版本时:续录遇到硬件不一致只**打一条告警并保留原文件**——告警滚过去就没了,
    盘上没有任何东西记下换过硬件,而清单从此把换后采的每一集都记在旧设备名下。
    换句话说,**旧版本不是"记不下换硬件",是"记错"**。

    读取端不用分版本处理:换硬件之前写的老文件(只有扁平 `units`)会被当成一个开口的
...
读回的通常比这高——SDK 0.1.9 附带的是 leader `1.2.2` / follower `1.1.5`,刷完读回的就是这两个号。
这是正常的,见[三套编号](#v21)。上面用 `LeaderGripper` 是因为这一步刷的是 master 镜像;从夹爪换
