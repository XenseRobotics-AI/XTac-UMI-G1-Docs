...
After refitting, rerun the tactile and camera preview self-check (see [Preview before recording](recording.md#preview)). If the image is clearly worse, check whether the sensor is seated firmly, dirty or fitted backwards; encoder zero calibration does not fix image problems.
...
- Calibration is one-off: the zero and the travel limit are written to MCU flash, survive power cycles and do not need redoing on another host. Day to day, only confirm that `gripper.pos`≈0 closed and ≈1.0 fully open; see [Gripper calibration](calibration.md#41).
...
- Do a quick self-check before every collection batch (see the [Quickstart self-check](quickstart.md#self-check)).
