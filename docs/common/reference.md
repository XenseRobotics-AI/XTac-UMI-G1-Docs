...
| **数采背包** | 背包版的采集端:RK3588 计算节点,夹爪与头显都接到它上面,XTac-UMI Collector 在它上面运行;正式名 XTac-UMI 数采背包,见[认识数采背包](../product/backpack.md) |
| **XTac-UMI Collector** | 跑在数采背包上的采集软件,接入相机、夹爪 MCU 与头显位姿,负责录制、回放、导出与设备管理;版本基线 0.3.9,以设备系统页显示为准 |
...
| **XC** | 腕部 UVC 相机,序列号 `XC...` |
| **XTac-UMI XR** | 装在 Pico4 头显上的 VR 客户端 APP,把头显与追踪器位姿送到采集端,启动瞬间建立世界坐标系;见 [Pico4 头显与追踪器配置](pico4.md) |
...
- 完整报错与相关日志,不要截断。
- 设备身份:背包版抄控制台 系统 → 设备信息 页的设备 SN、采集端版本与每只夹爪的 SN / 固件版本;PC 版给 `scan_grippers` 的 side / role / firmware_sn 输出和[如何查版本](../pc/versions.md#check-versions)里那条命令的输出。
