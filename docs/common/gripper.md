...
本页讲 XTac-UMI G1 主 / 从夹爪怎么接、接好后怎么核对识别、两个按键和指示灯各是什么意思、序列号怎么分左右。夹爪本体在两种形态里完全一样,只有「线接到哪、识别怎么核、按键与灯语」不同,那几处用「背包版 / PC 版」标签页分开写。产品定位与系统组成见[认识 XTac-UMI G1](../product/g1.md),规格见[技术参数](../product/specs.md#specs)。
...
两个侧边按键用于录制控制,指示灯反馈当前状态。背包版由背包上的 Collector 接管按键与灯语,是现场推荐的录制控制方式;PC 版目前用命令行控制录制,按键映射仍在完善。
...
=== "背包版"
    ...
    | 确认态再**双击** | 删除上一条(灯紫常亮 3 秒;随后 3 秒冷却期内双击无效) |
    | 确认态**左爪长按** | 取消删除 |
    | 确认态**右爪长按** | 放弃删除并立即开始新录制(上一条保留) |

    绑定爪位(右开 / 左停)、键位时序与灯语是全机队固定值,现场不可调;当前生效值在控制台 系统 → 按键绑定 页查看。
...
=== "背包版"
    ...
    | 紫闪 | 等待删除确认 |
    | 紫常亮 | 已删除 |
    | 红常亮 | 系统问题(如夹爪掉线,由背包侧点亮) |
    | 红爆闪 | 夹爪端问题(夹爪 MCU 自检点亮) |
