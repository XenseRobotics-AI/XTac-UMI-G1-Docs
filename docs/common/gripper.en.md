...
This page covers how to connect the XTac-UMI G1 leader and follower grippers, how to verify they are detected, what the two buttons and the indicator LED mean, and how the serial number tells left from right. The gripper itself is identical in both editions; only where the cable goes, how detection is checked, and the button and LED semantics differ, and those parts are split into "Backpack Kit / Developer Kit" tabs. Product positioning and the system components are in [XTac-UMI G1](../product/g1.md); specifications in [Specifications](../product/specs.md#specs).
...
## Buttons and LEDs {#buttons-leds}

The two side buttons control recording and the LED reports the current state. On the Backpack Kit the Collector on the backpack owns the buttons and the LED patterns, and this is the recommended way to drive recording in the field; on the Developer Kit recording is controlled from the command line for now and the button mapping is still being finalised.
...
=== "Backpack Kit"
    ...
    | **Double-press** again while confirming | Delete the previous episode (solid purple for 3 s; double-presses are ignored during the 3 s cool-down that follows) |
    ...
    Which gripper does what (right starts / left stops), the press timings and the LED patterns are fixed fleet-wide and cannot be changed on site; the values in effect are shown on the console's System → Key bindings page.
...
=== "Backpack Kit"
    ...
    | White blink | Saved normally |
    | Solid yellow | Failed to start recording / fatal problem while recording |
    | Pulsing yellow | Data suspect (saved, but that episode has a problem) |
    | Fast yellow blink | Action refused |
    | Purple blink | Waiting for delete confirmation |
    | Solid purple | Deleted |
    | Solid red | System problem (e.g. a gripper dropped off; lit by the backpack side) |
    | Red strobe | Gripper-side problem (lit by the gripper MCU's self-check) |
