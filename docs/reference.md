...
查表用的附录:`RobotConfig` 配置项、术语表、SDK 入口、反馈渠道与相关仓库;采集流程见[数据采集](recording.md),报错见[故障排查](troubleshooting.md)。
...
| **6D rotation** | 旋转矩阵 R(世界 ← 本体)的**前两列**:`r1..r3` = 第一列 = 本体 X 轴在世界系下的方向,`r4..r6` = 第二列 = Y 轴;第三列为叉积,可算回 → [约定详解](recording.md#53) |
...
1. 先查[故障排查](troubleshooting.md)(含常见问题)。
...
- [如何查版本](versions.md#check-versions)里那条命令的输出。
...
| 资源 | 地址 | 说明 |
|---|---|---|
| 数采主仓库 `xense-taccap-lerobot` | <https://github.com/XenseRobotics-AI/xense-taccap-lerobot> | 附带设备说明与 CHANGELOG |
| 夹爪 SDK `TacCap-Gripper` | <https://github.com/XenseRobotics-AI/TacCap-Gripper> | 子模块 `third_party/taccap-gripper/`;SDK 文档在 `docs/` |
| 追踪器 PC 服务 `XenseVR-PC-Service` | <https://github.com/XenseRobotics-AI/XenseVR-PC-Service> | 不是子模块,以 `.deb` 安装到 `/opt/apps/roboticsservice`,见[一键安装](install.md#24) |
...
本手册只覆盖从数采夹爪硬件到数据落盘为 `LeRobotDataset` 的链路;模型训练、推理、部署不在范围内,见对应工程的独立文档。
